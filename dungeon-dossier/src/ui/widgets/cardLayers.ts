...
export function createCardAttachments(
  patch: Partial<CardAttachments> = {},
): CardAttachments {
  return {
    ...(patch.stampId === undefined ? {} : { stampId: patch.stampId }),
    ...(patch.postId === undefined ? {} : { postId: patch.postId }),
    evidenceIds: [...(patch.evidenceIds ?? [])],
  };
}

export function attachStamp(
...
export function detachStamp(attachments: CardAttachments): CardAttachments {
  if (attachments.stampId === undefined) return attachments;
  return {
    ...(attachments.postId === undefined ? {} : { postId: attachments.postId }),
    evidenceIds: [...attachments.evidenceIds],
  };
}

export function detachPost(attachments: CardAttachments): CardAttachments {
  if (attachments.postId === undefined) return attachments;
  return {
    ...(attachments.stampId === undefined ? {} : { stampId: attachments.stampId }),
    evidenceIds: [...attachments.evidenceIds],
  };
}
...
export function buildCardLayerStack(
  ...
  if (attachments.stampId !== undefined) {
    slots.push({
      layer: 'stamp',
      zIndex: CARD_LAYER_Z_INDEX.stamp,
      attachmentId: attachments.stampId,
    });
  }
  if (attachments.postId !== undefined) {
    slots.push({
      layer: 'post',
      zIndex: CARD_LAYER_Z_INDEX.post,
      attachmentId: attachments.postId,
    });
  }
