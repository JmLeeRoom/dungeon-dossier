...
본 디렉토리(`docs/phase/`)는 **[던전 수사 조서 (Dungeon Detective Kim Taehoon)]** 개발을 위한 단계별 프롬프트 모음입니다.
기획자(Game Designer)가 **코드 수정 없이 JSON 데이터를 추가/수정**하고, **이미지 및 오디오 애셋을 인기술 UI 상에 직접 드래그 앤 드롭하여 게임을 제작/튜닝**할 수 있는 아키텍처를 AI 코딩 에이전트(Claude, GPT-4o, Antigravity 등)가 완전 자동 생성 및 검증하도록 설계되었습니다.
